                      ...
                      <section className="pt-8 border-t border-border space-y-4">
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="flex items-center justify-between">
                              <div>
                                <p className="text-[10px] font-black uppercase tracking-tight text-text-main">Autoplay</p>
                                <p className="text-[8px] font-bold text-text-muted uppercase">Start automatically</p>
                              </div>
                              <button 
                                onClick={() => updateStyle({ 
                                  autoplayConfig: { 
                                    ...style.autoplayConfig, 
                                    enabled: style.autoplayConfig?.enabled === false ? true : false 
                                  } 
                                })}
                                className={cn(
                                  "w-10 h-5 rounded-full relative transition-colors duration-300",
                                  style.autoplayConfig?.enabled !== false ? "bg-primary" : "bg-black/10"
                                )}
                              >
                                <div className={cn(
                                  "absolute top-0.5 size-4 rounded-full bg-white transition-all duration-300 shadow-sm",
                                  style.autoplayConfig?.enabled !== false ? "left-5" : "left-0.5"
                                )} />
                              </button>
                           </div>

                           {/* Autoplay Speed */}
                           {style.autoplayConfig?.enabled !== false && (
                             <motion.div 
                             ...
                             >
                                <p className="text-[8px] font-black uppercase tracking-widest text-text-muted italic">Pace of the story</p>
                                <div className="flex p-1 bg-black/5 dark:bg-white/5 rounded-lg">
                                  {[
                                    { id: 'slow', title: 'Slow', ms: 6000 },
                                    { id: 'medium', title: 'Medium', ms: 4000 },
                                    { id: 'fast', title: 'Fast', ms: 2500 }
                                  ].map((speed) => (
                                    <button
                                      key={speed.id}
                                      onClick={() => updateStyle({ 
                                      ...
                                      className={cn(
                                        ...
                                        (style.autoplayConfig?.speed || 4000) === speed.ms ? "bg-white shadow-sm text-primary" : "text-text-muted hover:text-text-main"
                                      )}
                                    >
                                      {speed.title}
                                    </button>
                           ...
                           <div className="flex items-center justify-between">
                              <div>
                                <p className="text-[10px] font-black uppercase tracking-tight text-text-main">Muted Start</p>
                                <p className="text-[8px] font-bold text-text-muted uppercase">Wait for unmute</p>
                              </div>
                              <button 
                                onClick={() => updateStyle({ 
                                  autoplayConfig: { 
                                    ...style.autoplayConfig, 
                                    muteOnStart: style.autoplayConfig?.muteOnStart === true ? false : true 
                                  } 
                                })}
                                className={cn(
                                  "w-10 h-5 rounded-full relative transition-colors duration-300",
                                  style.autoplayConfig?.muteOnStart === true ? "bg-primary" : "bg-black/10"
                                )}
                              >
                                <div className={cn(
                                  "absolute top-0.5 size-4 rounded-full bg-white transition-all duration-300 shadow-sm",
                                  style.autoplayConfig?.muteOnStart === true ? "left-5" : "left-0.5"
                                )} />
                              </button>
                           </div>
                        </div>
